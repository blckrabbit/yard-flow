...
export default function(G6){
  ...
  G6.registerBehavior('itemAlign', {
    ...
    _itemAlign(item){
      ...
      const cb = { x: bbox.x + bbox.width / 2, y: bbox.y + bbox.height-1 };
      const lc = { x: bbox.x, y: bbox.y + bbox.height / 2 };
      const rc = { x: bbox.x + bbox.width-1, y: bbox.y + bbox.height / 2 };
      const nodes = item.attrs && item.attrs.nodeId ? this.graph.getNodes().filter(n => n.get('id') !== item.attrs.nodeId) : this.graph.getNodes();
      each(nodes, (node) => {
        ...
        each(this.getHorizontalLines(bbox1), (line) => {
          ...
          // horizontalLines.push(this.getDistance(line,cc)); //注释中间线
        ...
        //   // verticalLines.push(this.getDistance(line,cc));  //注释中间线
