...
import {getGuid32L} from '../util/utils'
  ...
  initCommands(){
    ...
    cmdPlugin.registerCommand('redo', {
      ...
      execute: function(graph) {
        ...
        manager.current++;
        (window as any).cb.events.trigger('_graphCommand','redo',graph,manager);
      },
    ...
    cmdPlugin.registerCommand('undo', {
      ...
      execute: function(graph) {
        ...
        manager.current--;
        (window as any).cb.events.trigger('_graphCommand','undo',graph,manager);
      },
    ...
    cmdPlugin.registerCommand('paste', {
      ...
      method: function(graph) {
        ...
        const id = getGuid32L(); //clazz + timestamp;
        addModel.id = id;
